// Please refrain from tampering with the setup code provided here,
// as the index.html and test files rely on this setup to work properly.
// Only add code (helper methods, variables, etc.) within the scope
// of the anonymous function on line 6

const polybiusModule = (function () {
  // you can add any code you want within this function scope

  function polybius(input, encode = true) {
    // case sensitive inputs
    let lowerCasedInput = input.toLowerCase();
    let inputCount = input.replace(" ", "");
    let message = "";

    // all encoded values
    let encodeCode = {
      a: 11,
      b: 21,
      c: 31,
      d: 41,
      e: 51,
      f: 12,
      g: 22,
      h: 32,
      i: 42,
      j: 42,
      k: 52,
      l: 13,
      m: 23,
      n: 33,
      o: 43,
      p: 53,
      q: 14,
      r: 24,
      s: 34,
      t: 44,
      u: 54,
      v: 15,
      w: 25,
      x: 35,
      y: 45,
      z: 55,
    };

    // all decoded values
    let decodeCode = {
      11: "a",
      21: "b",
      31: "c",
      41: "d",
      51: "e",
      12: "f",
      22: "g",
      32: "h",
      42: "(i/j)",
      52: "k",
      13: "l",
      23: "m",
      33: "n",
      43: "o",
      53: "p",
      14: "q",
      24: "r",
      34: "s",
      44: "t",
      54: "u",
      15: "v",
      25: "w",
      35: "x",
      45: "y",
      55: "z",
    };

    if (encode) {
      // if encode === true then
      for (let i = 0; i < input.length; i++) {
        // loop over input array
        if (input[i] === " ") {
          // accounts for spaces
          message += input[i];
        }
        for (let letterPair in encodeCode) {
          // letterPair = num keys of encodeCode
          if (lowerCasedInput[i] === letterPair) {
            message += encodeCode[letterPair]; // lowerCasedInput[i] = letterPair
          }
        }
      }
    } else if (inputCount.length % 2 === 0) {
      // otherwise
      for (let i = 0; i < input.length; i += 2) {
        // loops through each pair
        if (input[i] === " ") {
          // accounts for spaces
          message += input[i]; // lowerCasedInput[i] = input[i];
        }
        let slicedNumbers = inputCount.slice(i, i + 2);
        for (let num in decodeCode) {
          if (slicedNumbers === num) {
            // compares strings from slice
            message += decodeCode[num]; // final message decoded from inputs
          }
        }
      }
    } else {
      return false;
    }
    return message; // return final message
  }
  return {
    polybius,
  };
})();

module.exports = { polybius: polybiusModule.polybius };
